...
def send_telegram_alert(message):
    subscribers = load_subscribers()
    if not subscribers:
        return False, "No subscribers found. Please add Chat IDs first."
    success_count = 0
    failed = []
    for chat_id in subscribers:
        ...
        try:
            ...
            failed.append(chat_id)
    return True, f"Sent to {success_count} users. Failed: {len(failed)}"
...
if st.session_state.prediction_done:
    ...
    # Risk Score Graph - smaller width
    ...
    col_left, col_right = st.columns([1.2, 1])  # Graph ochi width ma
    
    with col_left:
        fig_impact = px.bar(impact_df, x="System", y="Risk Score", color="Level",
...
if not history_df.empty:
    ...
    fig_prob = px.line(history_df, x="timestamp", y="storm_prob", 
...
# ====================== TELEGRAM SECTION ======================
st.markdown("---")
st.subheader("4. Telegram Multi-User Alert System")

if st.session_state.prediction_done:
    st.code(st.session_state.telegram_msg)

subscribers = load_subscribers()
st.write(f"**Current Subscribers:** {len(subscribers)}")
if subscribers:
    st.write(subscribers)

st.markdown("##### Add New Subscriber")
new_chat_id = st.text_input("Enter Chat ID to add")
if st.button("➕ Add Subscriber"):
    if new_chat_id.strip():
        added = save_subscriber(new_chat_id.strip())
        if added:
            st.success(f"Chat ID {new_chat_id} added successfully!")
            st.rerun()
        else:
            st.info("This Chat ID is already in the list.")
    else:
